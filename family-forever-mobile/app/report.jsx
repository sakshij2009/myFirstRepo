...
import CriticalIncidentModal from "../src/components/CriticalIncidentModel"
...
const [showMedicalContact, setShowMedicalContact] = useState(false);
const [showNoteworthy, setShowNoteworthy] = useState(false);
...
const fetchIntakeForm = async () => {
  try {
    ...
    if (matchedIntake) {
      ...
      alert("No intake form mapped to this client");
    }
  } catch (err) {
    console.log("Error fetching intake form:", err);
  ...
  onPress={async () => {
    ...
    if (!intakeData) await fetchIntakeForm();
  }}
>
  <Text style={{ color: "#2563EB", fontWeight: "700", marginTop: 6 }}>
    {showIntake ? "Hide Intake Form" : "View Intake Form"}
  </Text>
</Pressable>
  ...
  {showIntake && intakeData && <IntakeView intakeData={intakeData} />}
    ...
    <>
      ...
      <View style={{ marginHorizontal: 16, marginTop: 26 }}>
       ...
       <ActionCard
        ...
        color="#2563EB"
        icon="account"
        onPress={() => setShowMedicalContact(true)}
      />
        ...
        color="#F97316"
        icon="note-text"
        onPress={() => setShowNoteworthy(true)}
      />
       ...
       <ActionCard
        ...
        color="#16A34A"
        icon="repeat"
        onPress={() => setShowFollowThrough(true)}
      />
  ...
  visible={showMedicalContact}
  onClose={() => setShowMedicalContact(false)}
  shift={shift}
  onSubmit={(payload) => {
    console.log("MEDICAL CONTACT LOG:", payload);
    // next step: save to Firestore
  }}
/>

<NoteworthyEventModal
  visible={showNoteworthy}
  onClose={() => setShowNoteworthy(false)}
  shift={shift}
  onSubmit={(payload) => {
    console.log("NOTEWORTHY EVENT:", payload);
  }}
/>

<FollowThroughModal
  visible={showFollowThrough}
  onClose={() => setShowFollowThrough(false)}
  shift={shift}
  onSubmit={(payload) => {
    console.log("FOLLOW THROUGH:", payload);
  }}
/>
...
<CriticalIncidentModal
  ...
  shift={shift}
  onSaveDraft={(payload) => console.log("CRITICAL DRAFT:", payload)}
  onSubmit={(payload) => console.log("CRITICAL SUBMIT:", payload)}
/>
