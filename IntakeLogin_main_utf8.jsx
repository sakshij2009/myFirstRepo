...
const IntakeLogin = () => {
  ...
  useEffect(() => {
    if (isSignInWithEmailLink(auth, window.location.href)) {
      // 1∩╕ÅΓâú Try reading email from URL query param (set when admin/user sent the link)
      const urlParams = new URLSearchParams(window.location.search);
      let storedEmail = urlParams.get("email");

      // 2∩╕ÅΓâú Fall back to localStorage (same-device self-login flow)
      if (!storedEmail) {
        storedEmail = window.localStorage.getItem("emailForSignIn");
      }

      // 3∩╕ÅΓâú Last resort: ask the user (should almost never be needed now)
      if (!storedEmail) {
      ...
      <div className="flex flex-col justify-center items-center m-auto py-4 px-6 w-[404px] gap-6 bg-white shadow-xl rounded-xl">
        ...
        {isLogin ? (
          ...
          <>
            ...
            {/* AGENCY FIELD ΓÇö Always show for Intake Worker (which is now default) */}
            <InputField
